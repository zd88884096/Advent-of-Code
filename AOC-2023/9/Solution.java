...
public class Solution {
    public static boolean not_all_zero(List<Long> l){
        for(long ele : l){
            if(ele != 0L){
                return true;
            }
    ...
    public static void main(String[] args){
        ...
        for(String S : SS){
            ...
            l.add(nums);
            while(not_all_zero(l.get(l.size() - 1))){
                List<Long> next = new ArrayList<>();
                List<Long> last = l.get(l.size() - 1);
                for(int i = 0; i < last.size() - 1; ++i){
                    next.add(last.get(i + 1) - last.get(i));
                }
                l.add(next);
            }
            l.get(l.size() - 1).add(0L);
            for(int li = l.size() - 2; li >= 0; --li){
                List<Long> cur = l.get(li), next = l.get(li + 1);
                cur.add(cur.get(cur.size() - 1) + next.get(next.size() - 1));
            }
            long cc = l.get(0).get(l.get(0).size() - 1);
