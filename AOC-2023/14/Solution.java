...
public class Solution {
    static long res = 0L;
    static char[][] arr;
    public static long score(char[][] aa){
        int m = aa.length, n = aa[0].length;
        long rr = 0L;
        for(int i = 0; i < m; ++i){
            for(int j = 0; j < n; ++j){
                if(aa[i][j] == 'O'){
    ...
    public static char[][] tilt(){
        int m = arr.length, n = arr[0].length;
        int[] last = new int[n];
        Arrays.fill(last, -1);
        for(int i = 0; i < m; ++i){
            for(int j = 0; j < n; ++j){
                int target_col = last[j] + 1;
                if(arr[i][j] == 'O'){
                    arr[i][j] = '.';
                    arr[target_col][j] = 'O';
                    last[j] = target_col;
    ...
    public static void main(String[] args){
        ...
        int m = arr.length, n = arr[0].length;
        long cycle = 4000000000L;
        List<char[][]> l = new ArrayList<>();
        while(cycle > 1000){
            l.add(copy(arr));
            for(int i = 0; i < l.size() - 1; ++i){
                if(equal(l.get(i), l.get(l.size() - 1))){
                    int factor = (l.size() - 1 - i) * 4;
