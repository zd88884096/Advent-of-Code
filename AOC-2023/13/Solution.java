...
public class Solution {
    ...
    public static long score(char[][] arr){
        int m = arr.length, n = arr[0].length, sc = 0;
        int ct = 0;
        loop1: for(int line = 0; line < m - 1; ++line){
            for(int d = 0; d < m; ++d){
                int r1 = line - d, r2 = line + d + 1;
                if(r1 < 0 || r2 >= m){
                    break;
                }
                for(int j = 0; j < n; ++j){
        ...
        loop2: for(int line = 0; line < n - 1; ++line){
            for(int d = 0; d < n; ++d){
                int c1 = line - d, c2 = line + d + 1;
                if(c1 < 0 || c2 >= n){
                    break;
                }
                for(int i = 0; i < m; ++i){
    ...
    public static void solve(){
        ...
        long oric = score(arr);
        int ori = (int)(oric / 10000);
        for(int i = 0; i < arr.length; ++i){
            for(int j = 0; j < arr[0].length; ++j){
                flip(arr, i, j);
                long nscc = score(arr);
                int nsc = (int)(nscc / 10000), ctct = (int)(nscc % 10000);
                if(nsc > 0 && nsc != ori){
                    System.out.println(i + " " + j + " " + nsc);
                    if(nsc % 100 != 0 && nsc / 100 != 0 && (ori / 100 == nsc / 100 || ori % 100 == nsc % 100)){
                        nsc -= ori;
                    }
                    else if(ctct == 2){
                        nsc -= ori;
