...
public class Solution {
    ...
    public static void main(String[] args){
        ...
        while(i < n){
            ...
            List<long[]> map = l.get(l.size() - 1);
            map.add(stol(split(S, " ")));
            ++i;
        }
        long[] st = l.get(0).get(0);
        for(int k = 0; k < st.length; k += 2){
            long s = st[k], r = st[k + 1];
            for(long sd = s; sd < s + r; ++sd){
                long seed = sd;
                loop: for(i = 1; i < l.size(); ++i){
                    List<long[]> map = l.get(i);
                    for(long[] mm : map){
                        if(seed >= mm[1] && seed < mm[1] + mm[2]){
                            seed = mm[0] + (seed - mm[1]);
                            continue loop;
                        }
                    }
