...
public class Solution {
    ...
    public static void main(String[] args){
        ...
        for (int i = 0; i < n; ++i){
            String S = SS[i];
            String[] toks = trim(S.split("[|]"));
            int[] winning = stoi(split(toks[0], " ")), nums = stoi(split(toks[1], " "));
            HashSet<Integer> winning_set = setify(winning);
            long score = 0;
            for(int ele : nums){
                if(winning_set.contains(ele)){
                    ++score;
                }
            }
            sum += count[i];
            for(int j = i + 1; j < Math.min(n, i + 1 + score); ++j){
                count[j] += count[i];
            }
