...
public class Solution {
    ...
    public static int toCoord(int i, int j){
        return i * 10000 + j;
    }
    ...
    public static void main(String[] args){
            ...
            while(j < n){
                ...
                if(num){
                    for(int ti = i - 1; ti <= i + 1; ++ti){
                        for(int tj = j_st - 1; tj <= j_end + 1; ++tj){
                            if(ti >= 0 && tj >= 0 && ti < m && tj < n && (!isNum(arr[ti][tj])) && arr[ti][tj] == '*'){
                                map.putIfAbsent(toCoord(ti, tj), new HashMap<>());
                                map.get(toCoord(ti, tj)).put(toCoord(i, j_st), val);
                            }
                        }
                    }
                }
        ...
        for(int ele : map.keySet()){
            HashMap<Integer, Integer> mm = map.get(ele);
            if(mm.size() == 2){
                long prod = 1;
                for(int e2 : mm.keySet()){
                    prod *= mm.get(e2);
                }
                sum += prod;
            }
