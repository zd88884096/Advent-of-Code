...
public class Solution {
    public static long res = 0L, last = 0L;
    public static char[] steps;
    public static HashMap<String, PP> map = new HashMap<>();
    public static class PP{
        String a, b;
        public PP(String ap, String bp){
            a = ap;
            b = bp;
        }
    }
    public static int[] arr = {12083, 18827, 20513, 13207, 17141, 22199};
    public static boolean all_EZ(List<String> l){
        int st = 0;
        boolean bad = true;
    ...
    public static long solve(){
        ...
        while(!all_EZ(l)){
            for(int i = 0; i < l.size(); ++i){
                String cur = l.get(i);
                if(steps[(int)(res % (long)steps.length)] == 'L'){
                    cur = map.get(cur).a;
                }
                else{
                    cur = map.get(cur).b;
            ...
            if(res % 10000000 == 0){
                System.out.println(res / 10000000);
            }
            ++res;
        }
        return res;
    ...
    public static void main(String[] args){
        ...
        steps = SS[0].toCharArray();
        for(int i = 2; i < SS.length; ++i){
            String[] toks = split(SS[i], " ");
            map.put(toks[0], new PP(toks[1], toks[2]));
        }
