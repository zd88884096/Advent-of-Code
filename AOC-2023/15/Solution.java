...
public class Solution {
    ...
    public static void main(String[] args){
        FILENAME = args[0];
        String[] toks = read_all_String();
        System.out.println("TOK LENGTH: " + toks.length);
        toks = split(toks[0], ",");
        List<PP>[] l = new List[256];
        for(int i = 0; i < 256; ++i){
            l[i] = new ArrayList<>();
        }
        loop: for(String S : toks){
            int ii = S.indexOf("="), jj = S.indexOf("-");
            char[] arr;
            int power = 0;
            String[] tt;
            if(ii >= 0){
                tt = S.split("=");
                power = Integer.parseInt(tt[1]);
            }
            else{
                tt = S.split("-");
            }
            arr = tt[0].toCharArray();
            int hash = 0;
            for(char c : arr){
                hash += c;
                hash *= 17;
        ...
        for(int i = 0; i < 256; ++i){
            ...
            for(int j = 0; j < lc.size(); ++j){
                res += (long)(i + 1) * (long)(j + 1) * (long)lc.get(j).b;
            }
        }
