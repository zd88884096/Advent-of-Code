...
public class Solution {
    //idea: Part I: just multiplies 6 to number of blocks, then subtract touching surfaces with a trick
    //Part II: do bfs starting from (0, 0, 0) (outside the blocks range)
    //  and limit the search range to a bit wider than [0..30] for each direction, also not allowing bfs queue
    //  to contain coordinates of blocks.
    //  (30 - 5 (5 is for enough lee-way) is around the max coordinate for the input), the reason for us to do bfs is that we notice each outer surface
    //  will be touched by the location in our bfs queue exactly once (i.e. when some location (i, j, k) wants to expand to its neighbor (on the cardinal direction)
    //  (ti, tj, tk) and gets blocked by a block, we count the surface between (i, j, k) and (ti, tj, tk) once, 
    //  and since we never allowed our bfs queue to include (ti, tj, tk) as that it the coordinate of a block, we would never count this surface again
    //  since the only way we could do that is by having (ti, tj, tk) in queue and trying to go to (i, j, k)
    //
    //This bfs process of starting from outside ensures we never enter the interior or surface of the blocks, meaning we only count each surface area of blocks once.
    //  Leading us to the desired answer.
    ...
    public static void main(String[] args){
        ...
        for(String S : read_all_String()){
            ...
            //compress coordinates and add to "blocks"
            blocks.add(arr[0] * MAX * MAX + arr[1] * MAX + arr[2]);
            for(int i = 0; i < 3; ++i){
                max[i] = Math.max(max[i], arr[i]);
            }
            ct += 6;
        }
        //if block ele has 2 coordinate the same as e2 and the different coordinate only differs by 1, they are adjacent
        //--ct, later when we iterates e2 in the outer loop and ele in the inner, we reduce ct by 1 again (so that
        //  accomplishes our goal to reduce ct by 2 for each touching surface)
        for(long ele : blocks){
            for(long e2 : blocks){
                long diff = Math.abs(ele - e2);
                if(diff == MAX * MAX || diff == MAX || diff == 1){
                    --ct;
                }
            }
        }
        ...
        int ct2 = 0;
        Queue<Triple<Long, Long, Long>> q = new LinkedList<>();
        HashSet<Long> visited = new HashSet<>();
        visited.addAll(blocks);
        q.add(new Triple<>(0L, 0L, 0L));
        visited.add(0L);
        ...
        loop: while(!q.isEmpty()){
            int sz = q.size();
            for(int s = 0; s < sz; ++s){
                Triple<Long, Long, Long> t = q.poll();
                long x = t.a, y = t.b, z = t.c;
                long coord = x * MAX * MAX + y * MAX + z;
                for(int i = 0; i < dir.length; ++i){
                    long tx = x + dir[i][0], ty = y + dir[i][1], tz = z + dir[i][2];
                    if(tx < -1 || ty < -1 || tz < -1 || x > 30 || y > 30 || z > 30){
                        continue;
                    }
                    long ele = tx * MAX * MAX + ty * MAX + tz;
                    //going from open location to a block -> touched a surface area, add 1 to ct2
                    if(blocks.contains(ele)){
                        ++ct2;
                    }
                    if(!visited.contains(ele)){
                        visited.add(ele);
                        q.add(new Triple<>(tx, ty, tz));
                    }
                }
            }
        }
        System.out.println("Task 2: " + ct2);
    }
