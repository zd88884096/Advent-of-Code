...
public class Solution {
    public static void one_cycle(int change){
        board[i][j] = (x - 1 <= j && j <= x + 1) ? '#' : '.';
        ++cycle; ++j;
        if(cycle % 40 == 20){
            sum += cycle * x;
        }
    ...
    public static int cycle = 0, sum = 0, x = 1, i = 0, j = 0;
    public static char[][] board;
    
    @SuppressWarnings("unchecked")
    public static void main(String[] args){
        //idea: simulate, i, j are iterating indices for the board, "sum" is the sum for part I
        board = new char[6][40];
        for(String S : read_all_String()){
            String[] toks = split(S, " ");
            one_cycle(0);
            if(toks.length == 2){
                one_cycle(Integer.parseInt(toks[1]));
            }
        }
