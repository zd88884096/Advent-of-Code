...
//Optimized more using some ideas from https://www.reddit.com/r/adventofcode/comments/zpihwi/2022_day_19_solutions/

public class Solution_Extra_Optimized { 
    ...
    static long[] robots, minerals, cost_max;
    ...
    public static void dfs(long time){
        ...
        for(int i = 0; i < 4; ++i){
            if(minerals[i] < cost[3][i]){
                candidate_new_robo[3] = false;
                break;
            }
        }
        
        if(candidate_new_robo[3]){
        ...
        loop: for(int r = 0; r < 4; ++r){
            ...
            if(r < 3 && minerals[r] >= time * cost_max[r]){
                //System.out.println(minerals[r] + " " + time + " " + cost_max[r]);
        ...
        if((!candidate_new_robo[3]) && (ore < 6))
        //dfs call for adding no robot
            dfs(time - 1);
    ...
    public static long solve(String S, long T, boolean count_id){
        ...
        for(int i = 0; i < 4; ++i){
            for(int j = 0; j < 4; ++j){
                cost_max[i] = Math.max(cost_max[i], cost[j][i]);
            }
        }
        //print(cost_max);
