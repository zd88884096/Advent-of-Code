...
public class Solution {
    //idea: Basic simulation, to get the sum of all those numbers
    //  compute each number and add them up
    //To transform sum to SNAFU, parse it out digit by digit starting
    //  from the smallest digit, and for each digit if it's between
    //  3 and 4 (when taking sum % 5), we instead need a '=' and '-' digit
    //  respectively, but they have the effect of subtracting 5 at this digit
    //  so we add 1 to the next digit (one digit more significant than it)
    //  to balance out the effect (sum - 5 * d + 1 * (5 * d) = sum).
    ...
    public static void main(String[] args){
        ...
        for(String S : read_all_String()){
            ...
            long mult = 1L, res = 0L;
            //do what the problem asks and compute the value of each SNAFU number
            for(int i = s.length - 1; i >= 0; --i){
                res += mult * map.get(s[i]);
                mult *= 5;
            }
            sum += res;
        }
        ...
        //parse out sum as SNAFU number from smallest to largest digit
        while(sum != 0L){
            if(sum % 5 < 3){
                sb.append(sum % 5);
                sum /= 5;
                ...
                sb.append(sum % 5 - 5 == -1L ? '-' : '=');
                sum = sum / 5 + 1;
            }
        }
        //need to reverse the String before printing since we added
        //  digits to String in the order of smallest to largest digit
