...
public class Solution {
    ...
    public static void main(String[] args){
        //idea: Part I, just do regular BFS (with location of 'S' initially in q) and record the locations reachable for each step in q
        //Part II, just add all locations with elevation 'a' into the q to start with
        char[][] board = read_all_char_arr();
        row = board.length;
        col = board[0].length;
        Queue<Triple<Integer, Integer, Character>> q = new LinkedList<>();
        ...
        for(int i = 0; i < row; ++i){
            for(int j = 0; j < col; ++j){
                ...
                if(board[i][j] == 'S' || board[i][j] == 'a'){
                    visited[i][j] = true;
                    board[i][j] = 'a';
                    q.add(new Triple<Integer, Integer, Character>(i, j, 'a'));
                }
                else if(board[i][j] == 'E'){
                    board[i][j] = 'z';
                    ei = i;
        ...
        loop: while(!q.isEmpty()){
            ...
            for(int k = 0; k < sz; ++k){
                Triple<Integer, Integer, Character> p = q.poll();
                int i = p.a, j = p.b;
                char c = p.c;
                if(i == ei && j == ej){
                    break loop;
                ...
                for(int s = 0; s < 4; ++s){
                    ...
                    if(ti >= 0 && ti < row && tj >= 0 && tj < col && (int)board[ti][tj] <= c + 1 && !visited[ti][tj]){
                        visited[ti][tj] = true;
                        q.add(new Triple<>(ti, tj, board[ti][tj]));
                    }
                }
                //System.out.print(c + " ");
            }
        ...
        System.out.println("Task 2: " + step);
    }
