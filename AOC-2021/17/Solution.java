...
public class Solution {
    ...
    public static void main(String[] args){
        ...
        //  Also, we notice that if the point's initial velocity is (x, y), it eventually falls to the a height of 0
        //  (Since it's height's trajectory is y, y + (y - 1) ... sum(1..y), sum(1..y) - 1 = sum(2..y), sum(3..y), ... sum(y) = y, 0)
        //  Then the next second its y coordinate drops to -(y + 1), say the target y range is [-100, -50], any y s.t. -(y + 1) has no chance
        //  of hitting the target y range ever. (cause one second they are at 0 (above y range), next second it falls below the y range, then
        //  they keep falling). We ideally want y + 1 = 100, y = 99, as that just barely falls within the range after 199 seconds, but is the largest
        //  initial y we can find.
        //Now looking at the problem input, ideally we want the point's x-coordinate to stop within the range, which is entirely not so hard to do, since
        //  the point's stopping x-coordinate follows the form 1 + 2 ... + i, which is (1 + i) * i / 2, multiple value of this form falls between the input
        //  So apparently we know that our x-coord can always stop within the range, we just need to choose the maximum y (99 in this case) and compute
        //  (1 + 99) * 99 / 2

        //part II: we just bruce force hei hei
        //We need x to be able to reach the left boundary of x range, some optimization
        //min y velocity is the lower boundary of y range (otherwise we overshoot after one second), max y is -1 * lower bound y - 1 (just like part I)
        //max x is right boundary of x range, min x is high enough to reach left boundary of x range
        ...
        for(int x = 0; x <= arr[1]; ++x){
            //makes sure we reach the left boundary of x range
            if(top(x) < arr[0])
                continue;
            for(int y = arr[2]; y <= -1 * arr[2] - 1; ++y){

                //simulate
                int vy = y, vx = x, dy = 0, dx = 0;
                boolean in = false;

                //if we overshoot (will rather quickly, once y starts free-falling (actually at most
                //  arr[2] * -2 - 1 second, you can try to prove this or prove me wrong, just an accurate estimate)), 
                //  this point has no change of falling into the range again, break the while loop
                while(dy >= arr[2] && dx <= arr[1]){
                    //combined with the stuff in while loop, means (dx, dy) is in target range
                    if(dy <= arr[3] && dx >= arr[0]){
                        in = true;
                        break;
                    }
                    dx += vx;
