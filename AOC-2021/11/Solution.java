...
public class Solution {
    //idea: part I: Simulate, for each iteration, first increase all energy level by 1
    //  then do bfs (with queue q) with all locations with >= 10 energy level as starting point
    //  for each location in q, simulate flash (increase surrounding by 1), also add all location encountered in q
    //  to a visited set, so that we don't flash some location twice, in the end, set all visited locations' energy to 0
    @SuppressWarnings("unchecked")
    public static void main(String[] args){
        int[][] arr = read_all_int_arr();
        int row = arr.length, col = arr[0].length, res = 0, ans_1 = 0, ans_2 = 0;
        int epoch = 1000;
        ...
        for(int _e = 0; _e < epoch; ++_e){
            ...
            //collecting points that first flashes
            for(int i = 0; i < row; ++i){
                for(int j = 0; j < col; ++j){
                    ++arr[i][j];
                    if(arr[i][j] >= 10){
            ...
            while(!q.isEmpty()){
                ++ct;
                int cur = q.poll(), i = cur / col, j = cur % col;
                for(int s = 0; s < 8; ++s){
                    int ti = i + dir_all[s], tj = j + dir_all[s + 1];
                    //trick 1: if visited[ti][tj] is true, we won't even bother adding one to arr[ti][tj] (as it's already above 10, and we
                    //  already visited (ti, tj), so we wouldn't flash (ti, tj) again, adding arr[ti][tj] by 1 wouldn't affect our result 
                    //  whatsoever) (this is accomplished by lazy evaluation of &&).
                    //and if visited[ti][tj] is false and we also found out that arr[ti][tj] exceeds 10, (ti, tj) needs to be flash
                    //
                    //trick 2: since we wouldn't ever increment visited location, we can just set arr[ti][tj] = 0 now and it will never change
                    //  until next iteration, it saves us the step of setting all flashed location to energy 0 after bfs
                    if(ti >= 0 && ti < row && tj >= 0 && tj < col && (!visited[ti][tj]) && (++arr[ti][tj] >= 10)){
                        visited[ti][tj] = true;
                        q.add(ti * col + tj);
                        arr[ti][tj] = 0;
                    }
        ...
        System.out.println("Task 1: " + ans_1);
        System.out.println("Task 2: " + ans_2);
    }
