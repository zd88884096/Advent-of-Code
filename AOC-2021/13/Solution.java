...
public class Solution {
    public static long MAX = (long)(1e8 + 0.1), _timer_ind = 0;
    public static long[] _timer = new long[2];
    @SuppressWarnings("unchecked")
    public static void main(String[] args){
        //just simulate, record all compressed coordinates in a HashSet "set" then in each iteration
        //  if fold along y, this is the same as for all point with y-coord > y, let it's y-coord -= (y-coord - line) * 2 (go to line, then go the same
        //      distance beyond).
        //HashSet automatically removes duplicates, so just take set.size() after each iteration to get the number of points still present
        String[] input = read_all_String();
        int ind = 0;
        HashSet<Long> set = new HashSet<>();
        while(ind < input.length){
            String[] toks = split(input[ind], "[,]");
            ++ind;
            if(toks.length != 2){
                break;
            }
            int[] coord = stoi(toks);
            set.add(coord[1] * MAX + coord[0]);
        }
        while(ind < input.length){
            String[] toks = split(input[ind], "[ =]");
            long line = Long.parseLong(toks[3]);
            HashSet<Long> next = new HashSet<>();
            for(long ele : set){
                long y = ele / MAX, x = ele % MAX;
                if(toks[2].equals("y")){
                    if(y > line)
                        y -= (y - line) * 2;
                }
            ...
            //print the number of points once after each fold
            System.out.println(set.size());
            ++ind;
        }

        //drawing...
        //10 and 50 are observed bounds, should be limited by minimum fold value for y and x respectively.
        char[][] arr = new char[10][50];
